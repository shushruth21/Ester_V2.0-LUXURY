-- Fix Model-Attribute Mappings for All Categories
...
-- 8. Set default values for each model attribute for smoother UX
-- Set default for Seat Width (22" as standard)
UPDATE public.model_attributes ma
SET default_value_id = av.id
FROM public.furniture_models fm
JOIN public.attribute_types at ON ma.attribute_type_id = at.id
JOIN public.attribute_values av ON av.attribute_type_id = at.id AND av.value = '22'
WHERE ma.model_id = fm.id
  AND at.name = 'Seat Width'
  AND ma.default_value_id IS NULL;

-- Set default for Seating Capacity (3 seater as standard)
UPDATE public.model_attributes ma
SET default_value_id = av.id
FROM public.furniture_models fm
JOIN public.attribute_types at ON ma.attribute_type_id = at.id
JOIN public.attribute_values av ON av.attribute_type_id = at.id AND av.value = '3'
WHERE ma.model_id = fm.id
  AND at.name = 'Seating Capacity'
  AND ma.default_value_id IS NULL;

-- Set default for Bed Size (queen as standard)
UPDATE public.model_attributes ma
SET default_value_id = av.id
FROM public.furniture_models fm
JOIN public.attribute_types at ON ma.attribute_type_id = at.id
JOIN public.attribute_values av ON av.attribute_type_id = at.id AND av.value = 'queen'
WHERE ma.model_id = fm.id
  AND at.name = 'Bed Size'
  AND ma.default_value_id IS NULL;

-- Set default for common attributes (using existing values)
UPDATE public.model_attributes ma
SET default_value_id = (
  SELECT av.id FROM public.attribute_values av 
  WHERE av.attribute_type_id = ma.attribute_type_id 
  ORDER BY av.sort_order LIMIT 1
)
WHERE ma.default_value_id IS NULL;