-- COMPREHENSIVE FURNITURE CONFIGURATION MIGRATION
...
-- Create attribute values for new attribute types
...
CROSS JOIN (
  ...
  ('storage_option', 'with_storage', 'With Storage', 1, 5000),
  ('storage_option', 'without_storage', 'Without Storage', 2, 0),
...
-- Update existing attribute values to match specifications
-- Update no_of_seats for SOFA BED
UPDATE public.attribute_values 
SET value = new_val.value, display_name = new_val.display_name
FROM (
  VALUES
  ('1_seat', '2_str', '2 Str'),
  ('2_seats', '3_str', '3 Str'),
  ('3_seats', '2_str_lounger_rhs', '2 Str+Lounger(RHS)'),
  ('2_3_c_seats', '2_str_lounger_lhs', '2 Str+Lounger(LHS)')
) AS new_val(old_value, value, display_name)
WHERE attribute_values.value = new_val.old_value
AND attribute_type_id IN (SELECT id FROM attribute_types WHERE name = 'no_of_seats');

-- Add additional no_of_seats values for SOFA BED
INSERT INTO public.attribute_values (id, attribute_type_id, value, display_name, sort_order, price_modifier, created_at)
SELECT 
  gen_random_uuid(),
  at.id,
...
-- Update fabric cladding plan values to match specifications
UPDATE public.attribute_values 
SET display_name = CASE 
  WHEN value = 'single_colour' THEN 'Single Colour'
  WHEN value = 'dual_colour' THEN 'Dual Colour'  
  WHEN value = 'tri_colour' THEN 'Tri Colour'
  ELSE display_name
END
WHERE attribute_type_id IN (SELECT id FROM attribute_types WHERE name = 'fabric_cladding_plan');

-- Add chair-specific foam values
INSERT INTO public.attribute_values (id, attribute_type_id, value, display_name, sort_order, price_modifier, created_at)
SELECT 
  gen_random_uuid(),
  at.id,
...
-- Add sofa bed specific leg values
INSERT INTO public.attribute_values (id, attribute_type_id, value, display_name, sort_order, price_modifier, created_at)
SELECT 
  gen_random_uuid(),
  at.id,
