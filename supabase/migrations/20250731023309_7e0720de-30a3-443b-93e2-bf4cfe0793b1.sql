-- Configure all SOFA ECONOMY models with comprehensive customization attributes

-- Insert model_attributes for all SOFA ECONOMY models
WITH sofa_economy_models AS (
  SELECT fm.id as model_id
  FROM furniture_models fm 
  JOIN categories c ON fm.category_id = c.id 
  WHERE c.name ILIKE '%sofa economy%'
),
attribute_mappings AS (
  SELECT 
    ...
    END as is_required,
    CASE 
      WHEN at.name = 'model' THEN (SELECT av.id FROM attribute_values av WHERE av.attribute_type_id = at.id AND av.value = 'Arbiter' LIMIT 1)
